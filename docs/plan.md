...
| 5 | 피험자 **00096** | 반팔 계열 최다 (`shortshort`, `shortlong`, `jerseyshort`) + **머리 길이 변화** |
| 6 | 필요 시 00032, 03375 | 반팔(`shortlong`, `shortshort`) 추가 |

의상 이름은 `<상의><하의>` 규칙이다 — `poloshort` = 폴로 + 반바지,
`shortshort` = 반팔 + 반바지, `longshort` = 긴팔 + 반바지. 프로젝트 범위가
**반팔 상의**이므로 첫 토큰이 `polo` / `short` / `jersey`인 조합만 의미가 있다.
`blazerlong`, `longlong`, `shirtlong`은 범위 밖이다.

00096·03284의 **머리 길이 변화**는 부수적 이득이 아니라 목표물이다. 결정 #32에서
Woman4의 어깨점이 머리카락 때문에 탐색 천장에 박혔는데, 그 실패를 재현할 수 있는
데이터가 여기 있다.
...
148,508프레임은 필요 없다. **모든 시퀀스가 A-pose로 시작한다**(03375의 일부
제외)고 명시돼 있고, 이 프로젝트의 SMPL canonical이 A-pose(`ABDUCTION_DEG=50`)다.
따라서 쓸모 있는 것은 **시퀀스 시작부의 A-pose 프레임**이지 동작 구간이 아니다.
연속 프레임은 거의 같은 관측이라 통계에 넣으면 N을 부풀린다.
...
| 2. CAPE 약관 원문 | ☑ 확보 (`cape-terms.txt`, provenance에 해시) |
| 3. SMPL 모델 라이선스 | ☑ |
| 4·5·7·8 | ☑ |
