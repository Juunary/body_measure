...
- **패턴 게이트가 신체 치수와 완제품 POM을 섞고 있다.** `chest_circumference`는
  ISO 8559-1 신체 치수, `hem_girth`·`sleeve_opening_girth`·`armhole_depth`는
  ISO 18890 계열 POM — 여유량이 들어간 뒤의 값이다. 게이트를 손대기 전에 이
  구분이 먼저다.
- **"unsourced" 3종의 근거 후보**가 드래프팅 교재가 아니라 ISO 18890 + 제조사
  POM 시트로 바뀌었다. 다만 그것들도 완제품 값이라 위 구분이 선행한다.
