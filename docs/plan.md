...
**승인 취득 (2026-09-01, 사용자 진술).** 위 제한에 대한 승인을 받았다고
전달받았다. LICENSE-G0의 규칙대로 **요약이 아니라 회신 원문**이 파일로 있어야
하고, 세 조항이 각각 별개이므로 어느 것이 승인됐는지 구분돼야 한다:

- [ ] 국외 소재 단체(ITA)의 **이용** 합의
- [ ] **국외 반출** 합의
- [ ] 제3자 **열람** 승인 (ITA 동료·공동연구자 포함)

`weights-permission.md` 규칙도 함께 걸린다 — 개인 앞으로 온 승인은 ITA를
구속하지도 보호하지도 않으므로, 수신 주체가 기관인지 확인이 필요하다.
증빙이 서면화되기 전까지 항목 상태는 ☑*(승인됨, 증빙 미완)로 둔다.
