...
### 허리는 띠의 중간, 엉덩이는 엉덩이 돌출 높이에서 (결정 #47, 2026-09-03)
studio에서 좌표를 찍어 올린 신고 셋이 계기: 고도비만 SMPL 몸은 둘레 최소가 없어
허리가 탐색 바닥에, 엉덩이가 배 아래에 잡혔고, Woman4는 최소점이 가슴 바로 밑이었다.
허리 = 둘레 최소점(늑골 아래)과 요추 오목점(등의 최소 후방 돌출)의 중간, 최소점이
경계면 오목점만. 엉덩이 = 엉덩이 최대 돌출 높이의 둘레. Texel 자연허리 높이 대비
평균 +25 → **+13mm**, 절대 35 → **19mm**. 참조는 m102 → m16(자연 허리, 5.3.10).
Man1(+47)·Woman4(+25)는 남는다 — 절반 개선이지 해결이 아니다.

### 위팔 둘레는 팔 축에 수직으로 (결정 #46, 2026-09-03)
studio에서 올라온 신고("A자 자세라 팔을 사선으로 잰다")가 맞았다. 수평 루프 중심점을
지나는 직선을 팔 축으로 삼고 그에 수직으로 자른다. SMPL A자세 342.5 → 308.7mm.
Texel 대조는 −3.3 → **−16.1mm**(sd 그대로), NOMO −5.5 → −13.6: 열 명이 같은 방향으로
같은 크기만큼 움직였으므로 수평 절단이 더하던 상수를 뺀 것이고, 참조값 자체가
수평 절단이었을 가능성이 크다. 어느 쪽이 맞는지는 ISO 8559-1 5.3.16 원문이 정한다
— `definition_verified`는 여전히 false.
