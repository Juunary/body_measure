...
원인: `true_gap_mm`이 **스칼라**(법선 성분)라 실제 옷의 접선 이동을 버린다.
팔에서 접선/법선 비가 **1.6배**이고 법선 성분 자체가 1~2mm뿐이라, 밴드가 가장
부실한 곳이 하필 정보가 가장 적은 곳이다.

**다음:** gap 항을 벡터로 만드는 것이 이 발견의 직접적 답이다. 그리고 CAPE
피험자가 **둘 다 남성**이라 젖가슴 구분을 시험하는 케이스가 하나도 없다 —
여성 피험자 다운로드가 그다음이다.
