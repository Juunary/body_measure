...
|---|---|---|---|---|---|---|
| waist_circumference | 최소 몸통 둘레 수평 슬라이스 / standing / 표면 둘레 | (조항 없음) | m102 "Minimum Waist Girth" | 없음 (Max/Trouser만) | **exact — 재검토 필요** | 명칭·정의 동일. m16(5.3.10 자연 허리선)은 정의 상이 → aux. **2026-09-07: 이 행은 spec v7 이전 상태다. `adapters/texel.py`는 결정 #47 이후 m16 을 참조로 쓰고 m102 는 aux 로 내려갔는데 — 코드와 이 표가 반대다. 판정(exact)은 헤드라인 통계에 들어가는 값을 정하므로 여기서 바꾸지 않고 표시만 한다. 정의 기준으로 다시 판정할 것** |
