...
| 우리 키 | ISO 조항 | 상태 |
|---|---|---|
| chest_circumference | 5.3.4 Bust girth / **5.3.6 Chest girth (at axilla)** | 후보 둘, **Texel 로는 구분 불가** — m5 와 m45 가 평균 10.4 mm 밖에 안 다르다 (결정 #55) |
...
## 가슴 정의 대조 (2026-09-08, 결정 #55)

Texel 이 두 참조를 다 싣고 있어 — m5 (5.3.4 Bust/Chest Girth), m45
(5.3.6 Chest Girth at axilla). 세 가지 산출 방식을 양쪽에 대조한 결과 (n=10):

| 산출 방식 | vs m5 (5.3.4) | vs m45 (5.3.6) |
|---|---|---|
| **search** — 현재 구현, 최대 몸통 둘레 | +26.9 / MAE **28.2** / sd 23.6 | +16.5 / MAE **22.3** / sd 23.2 |
| bust_level 높이 고정 (5.3.4 앵커) | −11.2 / MAE 42.4 / sd 65.5 / 최악 164.6 | −21.6 / MAE 53.0 |
| axilla_level 높이 고정 (5.3.6 앵커) | −46.6 / MAE 51.3 | −57.0 / MAE 64.7 |

| 참조 간 격차 | mean | MAE | sd | 최악 |
|---|---|---|---|---|
| m45 − m5 | +10.4 | 28.6 | 36.7 | 86.0 |

읽는 법 세 가지:

1. **두 참조가 서로를 구분해주지 않는다.** 평균 10.4 mm 차이라 우리가 어느
   조항에 대응하는지 이 데이터로는 못 정한다. 원문이 없어서가 아니라 데이터가
   그렇다.
2. **조항이 지정한 높이로 고정하면 오히려 멀어진다.** 겨드랑이 고정은 같은
   조항의 참조인 m45 에 대해서조차 세 방식 중 최악이다 (−57.0 / MAE 64.7).
3. **`estimate_bust_level` 이 불안정하다.** m5 대비 평균 −11.2 는 부호 상쇄의
   산물이고 (sd 65.5, Man3 −164.6 / Man4 +44.5), 이 랜드마크가 매 스캔에
   출력되는 `chest_max_exceeds_bust_level_girth_by_NNmm` 플래그의 근거다.

**주의:** Texel 의 m5·m45 배치 방법은 공개되지 않았다 (결정 #47 이 m43 에 대해
적어둔 것과 같은 상황). 스캐너 소프트웨어도 극값을 찾는다면 이 일치는 같은
방향의 오차 둘이고, "+26.9 mm 가 ISO 로부터의 이탈"이라는 읽기는 성립하지
않는다. 이 표는 어느 쪽이 옳은지가 아니라 **어느 쪽이 더 가까운지**만 말한다.

## 팔 클리핑 오차 (2026-09-08, 결정 #56)

가슴 행의 "클리핑은 구현 근사(별도 플래그)"에 처음으로 숫자가 붙었다. 정답은
SMPL 스키닝 가중치 — 병합 윤곽의 점을 관절 16–23(팔) 여부로 라벨링하고 팔
구간을 현으로 잇는다 (클리핑이 흉내 내는 줄자 규약 그대로). 클리핑 안 된 5개
바디에서 정답 = 측정 (−4 … +1 mm) 로 검산됨.

| 클리핑된 합성 4개 | mean | MAE | range |
|---|---|---|---|
| 측정 − 정답 (호 + 현) | **+2.5** | **14.0** | −22.9 … +17.0 |
| 측정 − 정답 (hull) | +7.3 | 16.0 | −17.4 … +23.2 |

Texel 10명에서 클리핑이 걷어내는 양은 182–250 mm (병합 둘레의 14–20 %).
**편향이 없으므로 Texel 대비 +26.9 mm 는 클리핑 오차가 아니다** — 남는 후보는
정의 차이(최대값 탐색 vs 비공개 참조 방식) 또는 참조 자체. n=4, 합성 바디는
실측보다 매끈하다는 한계는 결정 #56 에.

## 갱신 이력

- 2026-09-08 팔 클리핑 오차 측정 (결정 #56). 합성 4개에서 MAE 14.0, 무편향 — Texel
  +26.9 mm 는 클리핑이 아니라 정의/참조 문제로 좁혀짐
- 2026-09-08 가슴 정의 대조 (결정 #55). 5.3.4/5.3.6 은 Texel 로 구분 불가,
  조항 지정 높이로 고정하면 오히려 나빠짐, `estimate_bust_level` 불안정 발견
